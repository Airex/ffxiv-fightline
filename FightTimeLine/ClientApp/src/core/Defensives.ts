...
class MitigationVisitor implements M.IEffectVisitor {
        ...
        var context = {
            ...
            addMitigationForTarget(value: number, damageType: M.DamageType) {
                if (self.target) {
                    ...
                    if (self.damageType === M.DamageType.None || damageType == M.DamageType.All || ((damageType & self.damageType) === self.damageType))
    ...
    public build(holders: H.Holders) {
        var defStats = Object.keys(this.sums).map(s =>
        ({
...
export function calculateMitigationForAttack(holders: H.Holders, defs: DefsCalcResult, attack: { offset: string, type?: M.DamageType | number }): MitigationForAttack[] {
    var abs: DefsCalcResultAbility[] = defs.reduce((ac, j) => {
        return [...ac, ...j.abilities]
    }, []);

    const bossAttackStart = Utils.getDateFromOffset(attack.offset);

    var mitigationVisitor = new MitigationVisitor(holders);

    var used = new Set();
    abs.forEach(a => {
        const settingData = holders.itemUsages.get(a.id).getSettingData(SettingsEnum.Target);
        var target = settingData?.value || a.jobId;
