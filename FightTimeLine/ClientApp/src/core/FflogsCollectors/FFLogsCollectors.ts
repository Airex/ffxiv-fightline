...
import { BossAttackFFlogs, BossAttackFFlogsData } from "../Models"
...
export class BossAttacksCollector extends BaseCollector {
  ...
  process(): void {
    ...
    Object.keys(this.bossAttacks).forEach((k: string) => {
      const attk = this.bossAttacks[k];
      const ability = attk.find((it) => it.ability &&
      ...
      if (ability) {
        ...
        if (attk.length > 2) tags.push(M.DefaultTags[1])
        if (tbs.indexOf(ability.ability.name) >= 0) tags.push(M.DefaultTags[0])

        let fflogsData: BossAttackFFlogs = {};
        attk.forEach(at => {
          if (FF.isDamageEvent(at)) {
            const foundJob = this.context.parser.players.find(it1 => it1.id === at.targetID);
            if (!foundJob) return;
