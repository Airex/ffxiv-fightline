...
export abstract class BaseColumnTemplate {
    ...
    protected isOffsetNear(offset: TimeOffset, start: TimeOffset, distance: number) {
        const point = Utils.getDateFromOffset(offset, this.startDate);
        return Math.abs(Utils.getDateFromOffset(start, this.startDate).valueOf() - point.valueOf()) <= distance * 1000;
    }
