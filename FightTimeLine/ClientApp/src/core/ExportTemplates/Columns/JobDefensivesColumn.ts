...
import { AbilityType } from "src/core/Models";
...
export class JobDefensivesColumn extends BaseColumnTemplate implements IColumnTemplate<ExportAttack> {
    constructor(private it: ExportJob, private jobRegistry: IJobRegistryService, private afFilter: boolean, private coverAll: boolean, private showHealing: boolean) {
        super()
    }
    used = new Set<string>();
        ...
        const items = data.data.abilities.reduce((acc, ability) => {
            ...
            const condition = (this.coverAll || !this.used.has(ability.id)) &&
                ...
                (
                    ...
                    this.showHealing && isHealing && this.isOffsetNear(attack.offset, ability.start, 5)
                );
