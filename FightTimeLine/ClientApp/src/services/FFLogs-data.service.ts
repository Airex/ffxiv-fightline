...
export class FFLogsService implements IDataService {
  ...
  async getCached<T>(
    ...
    const item = cache.find(
      (it) =>
        it.key === itemKey &&
        new Date().valueOf() - it.timestamp.valueOf() <
          1000 * 60 * 60 * 24 * cacheDays
  ...
  async getFight(code: string): Promise<ReportFightsResponse> {
    return this.getCached<ReportFightsResponse>(
      "fights_cache",
      code,
      15,
      10,
      async () => {
        const url = `${this.fflogsUrl}v1/report/fights/${encodeURIComponent(
          code
        )}?translate=true&api_key=${this.apiKey}`;
  ...
  private loadFightChunk(
    ...
    filter: string,
    translate: boolean
  ): Observable<CorrectReportEventsResponse> {
    const url = `${this.fflogsUrl}v1/report/events/${code}?translate=${
      translate ? 1 : 0
    }&api_key=${
      this.apiKey
    }&start=${start}&end=${end}&actorinstance=${instance}&filter=${filter}`;
  ...
  async getEvents(
    ...
    const translate =
      config.fflogsImport.translate === undefined
        ? true
        : config.fflogsImport.translate;

    const f = await this.getFight(code);
    const parser = new Parser(instance, f);

    const resp = await this.getCached<BaseEventFields[]>(
      "events_cache",
      code + instance,
      10,
      10,
      async () => {
        ...
        const filter = parser.createFilter(
          this.jobRegistry,
          options?.bossAttacksOnly
        );
        ...
        do {
          ...
          const diff =
            (a.nextPageTimestamp || foundFight.end_time) -
            foundFight.start_time;
          const percentage =
            diff / (foundFight.end_time - foundFight.start_time);
          callBack(percentage);
  ...
  getZones(): Observable<Zone[]> {
    ...
    if (cache) {
      if (
        new Date().valueOf() - cache.timestamp.valueOf() <
        1000 * 60 * 60 * 24
      ) {
  ...
  getParses(
    ...
    const url = `${this.fflogsUrl}v1/parses/character/${encodeURIComponent(
      characterName
    )}/${encodeURIComponent(serverName)}/${encodeURIComponent(
      region
    )}?api_key=${this.apiKey}`;
