...
namespace FightTimeLine.Controllers
{
   ...
   public class DataController : Controller
   {
      ...
      [AllowAnonymous]
      public async Task<IEnumerable<BossSearchResult>> Bosses([FromRoute] long reference, [FromRoute] string value,
         [FromQuery] bool privateOnly, [FromRoute] string game)
      {
         ...
         return await _dataContext.Bosses
            .AsNoTracking()
            .Where(s => (string.IsNullOrEmpty(value) || EF.Functions.Like(s.Name, value)) && s.Reference == reference &&
                        s.Game == game && (!s.IsPrivate && !privateOnly || s.IsPrivate && s.UserName == name))
            .Select(s => new BossSearchResult()
            {
               Id = s.Identifier.ToString("N"),
               Name = s.Name,
               Owner = s.UserName,
               CanRemove = !string.IsNullOrWhiteSpace(name) &&
                           (string.Equals(name, s.UserName, StringComparison.OrdinalIgnoreCase) || isAdmin),
      ...
      public async Task<IActionResult> RemoveBosses([FromBody] string[] ids)
      {
         ...
         var data = await _dataContext.Bosses
            .Where(entity => guids.Contains(entity.Identifier) && (entity.UserName == nameClaim || user.IsAdmin))
            .ToArrayAsync().ConfigureAwait(false);
      ...
      public async Task<IActionResult> SaveFight([FromBody] FightData request)
      {
         ...
         if (fight != null)
         {
            if (fight.UserName.Trim() != nameClaim &&
                !string.Equals(fight.UserName, "anonymous", StringComparison.OrdinalIgnoreCase))
               return Unauthorized();
      ...
      public async Task<IActionResult> LoadCommands([FromRoute] string fight, [FromRoute] long? timestamp)
      {
         ...
         var query = _dataContext.Commands.AsNoTracking()
            .Where(entity => entity.Fight == fightValue);
